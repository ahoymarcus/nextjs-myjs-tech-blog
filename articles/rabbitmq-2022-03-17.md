...
6. ##### Hello World RabbitMQ and JavaScript
    6.1. About the Hello World RabbitMQ and JavaScript Tutorial
7. ##### Some RabbitMQ Server Actions
8. ##### 
9. #####
10. ##### Further Reading
11. ##### References
...
2. **Queues**: a queue is the name for a 'post box' or a internal storage  which lives inside the RabbitMQ server, and it is the only place where messages can take part in the flow between the producer and the consumer.  
...
This tutorial is using AMQP 0-g-1 and the Node-JS. And just bellow it is  installed the **amqp.node client** library: 
...
#### About the Hello World RabbitMQ and JavaScript Tutorial
...
### Some RabbitMQ Server Actions

Besides thoses actions used while installing and testing the RabbitMQ Server to see if its process is alive:

```
systemctl status rabbitmq-server.service
``` 

Here, how to see what queues RabbitMQ has and how many messages are in them:
