...
3. **Functions**: in the framework this term appllies to a group of tools and people which is used to perform one or more activities or processes.
...
10. **Baseline**: it can have two senses, being one refering to the start or initial point of a service or of a project.    
    10.1. But in this framework the term **baseline** is used for measuring how the operations of IT are performing.
...
13. **Process Manager**: in this framework is made a separation between a process owner and the process manager for better management.    
    13.1. The process manager role is to manage the operation of a processe.
...
15. **CI (Configuration Item)**: this is a term to be applied to any component which have a direct impact on the delivering of the IT and has to be specified and then store in the CMS (Cofiguration Management System).
...
17. **CMDB**: a component used for storing the configuration records for all the CIs and its relationships.
...
Here we have some interesting points from [JavaTPoint](https://www.javatpoint.com/itil) in why to use the ITIL framework:
